...
def test_mintable_in_timeframe(accounts, distributor, token, theoretical_supply, time, chain):
    ...
    t1 = chain[-1].timestamp
    available_supply = distributor.available_to_distribute()
    mintable = distributor.distributable_in_timeframe(t0, t1)
    assert (
